...
def align_timestamp_main(history_dir):
    df_text = pd.read_excel(get_cleaned_chunks_file(history_dir))
    df_text['text'] = df_text['text'].str.strip('"').str.strip()
    df_translate = pd.read_excel(get_translation_results_for_subtitles_file(history_dir))
    df_translate['Translation'] = df_translate['Translation'].apply(clean_translation)
    
    align_timestamp(df_text, df_translate, SUBTITLE_OUTPUT_CONFIGS, get_output_dir(history_dir))
    console.print(Panel("[bold green]🎉📝 Subtitles generation completed! Please check in the `output` folder 👀[/bold green]"))

    # for audio
    df_translate_for_audio = pd.read_excel(get_translation_results_remerged_file(history_dir)) # use remerged file to avoid unmatched lines when dubbing
    df_translate_for_audio['Translation'] = df_translate_for_audio['Translation'].apply(clean_translation)
    
    align_timestamp(df_text, df_translate_for_audio, AUDIO_SUBTITLE_OUTPUT_CONFIGS, get_audio_output_dir(history_dir))
