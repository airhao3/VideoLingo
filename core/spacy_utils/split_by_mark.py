...
def split_by_mark(nlp, history_dir):
    whisper_language = load_key("whisper.language")
    language = load_key("whisper.detected_language") if whisper_language == 'auto' else whisper_language # consider force english case
    joiner = get_joiner(language)
    print(f"[blue]🔍 Using {language} language joiner: '{joiner}'[/blue]")
    cleaned_chunks_path = os.path.join(history_dir, "log", "cleaned_chunks.xlsx")
    chunks = pd.read_excel(cleaned_chunks_path)
    ...
    with open(sentence_by_mark_path, "w", encoding="utf-8") as output_file:
    ...
    print(f"[green]💾 Sentences split by punctuation marks saved to →  `{sentence_by_mark_path}`[/green]")
